...
const touchJoystickMaxSpeedPxPerSecond=576;
...
const handleScummwebMessage=event=>{if(event.origin!==window.location.origin)return;if(event.data?.type==="scummweb-touch-click"){dispatchParentTouchClick(event.data.button);return}if(event.data?.type!=="scummweb-touch-joystick-state")return;setTouchJoystickState(event.data)};
